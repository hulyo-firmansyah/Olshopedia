...
class SettingController extends Controller
{
            ...
            if(strcmp($tipeKirim, $tipeKirim2) === 0){
                switch($tipeKirim){
                    ...
                    case 'tambah_supplier':
                        $cekEmail = DB::table('users')->where('email', $request->email)->get()->first();
                        if(isset($cekEmail)){
                    ...
                    case 'hapus_supplier':
                        $proses1 = DB::table('t_supplier')
                            ...
                            ->where('data_of', Fungsi::dataOfCek())
                            ->delete();
                        $proses2 = DB::table('users')
                            ->where('id', $request->userId)
                            ->delete();
                        $proses3 = DB::table('t_user_meta')
                            ->where('user_id', $request->userid)
                            ->where('data_of', Fungsi::dataOfCek())
                            ->delete();
                        Cache::forget('data_supplier_pengaturan_'.Fungsi::dataOfCek());
    ...
    public function getSupplierData(Request $request){
        if($request->ajax()){
            if(isset($request->v)){
                $supplier = DB::table('t_supplier')
                    ->join('t_user_meta', 't_supplier.user_id', '=', 't_user_meta.user_id')
                    ->join('users', 't_supplier.user_id', '=', 'users.id')
                    ->where('t_user_meta.role', 'Supplier')
                    ->where('users.id', $request->v)
                    ->where('t_supplier.data_of', Fungsi::dataOfCek())
                    ->select('t_supplier.*', 'users.no_telp', 'users.username', 'users.email')
                    ->get()->first();
            ...
            $supplier = Cache::remember('data_supplier_pengaturan_'.Fungsi::dataOfCek(), 10000, function(){
                return DB::table('t_supplier')
                    ...
                    ->join('users', 't_supplier.user_id', '=', 'users.id')
                    ->where('t_user_meta.role', 'Supplier')
                    ->select('t_supplier.*', 'users.no_telp')
                    ->get();
            });
            $data = [];
            $no = 0;
            foreach (Fungsi::genArray($supplier) as $row ) {
                ...
                $data[$no++] = [
                    ...
                    $row->no_telp,
                    $row->jalan,
                    "<input type='hidden' name='getSuppId' id='get-id' value='".$row->user_id."'>
                    <button type='button' data-target='#editSupplier' data-toggle='modal' class='btn btn-warning btn-xs' data-id='".$row->user_id."' onClick='$(this).editSupplier(tabelDataSupplier)'>Edit</button>
                    <button type='button' class='btn btn-danger btn-xs' data-id='".$row->user_id."' onClick='$(this).hapusSupplier(tabelDataSupplier);'>Hapus</button>",
                ];
            }
            $result['data'] = $data;
            return Fungsi::respon($result, [], 'json', $request);
